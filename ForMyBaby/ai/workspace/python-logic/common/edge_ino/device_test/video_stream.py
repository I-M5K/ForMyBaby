...
# 2. (예정) /dev/ttyACM0 시리얼 포트에서 데이터를 읽고 서버로 전송합니다.
# 3. (예정) /dev/video0 웹캠에서 프레임을 캡처하고 /dev/ttyACM0 시리얼 포트에서 데이터를 읽어 서버로 전송합니다.
# 4. (예정) mediapipe를 사용하여 웹캠에서 프레임을 캡처하고 서버로 전송합니다.
